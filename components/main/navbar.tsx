...
export const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-[#03001427] backdrop-blur-md shadow-lg shadow-[#2A0E61]/50 z-50">
      <div className="grid grid-cols-3 items-center h-[70px] px-5 md:px-10">
        {/* Logo & Name */}
        ...
        <div className="hidden md:flex justify-center">
          <div className="flex items-center gap-6 bg-[rgba(3,0,20,0.37)] px-6 py-2 rounded-full border border-[rgba(112,66,248,0.38)] text-gray-200">
            {NAV_LINKS.map((link) => (
              <Link 
                key={link.title} 
                href={link.link} 
                className="hover:text-[rgb(112,66,248)] transition font-medium text-sm"
              >
                {link.title}
              </Link>
            ))}
